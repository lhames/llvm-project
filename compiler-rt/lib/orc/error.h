...
private:
  ...
  void assertIsChecked() {
    if (ORC_RT_UNLIKELY(!isChecked() || getPtr())) {
      fprintf(stderr, "aborting due to unchecked error.\n");
      abort(); // Some sort of JIT program abort?
    }
  }

  template <typename ErrT = ErrorInfoBase> ErrorInfoBase *getPtr() const {
    return reinterpret_cast<ErrorInfoBase *>(ErrPtr & ~uintptr_t(1));
  }
...
private:
  ...
  void assertIsChecked() {
    if (ORC_RT_UNLIKELY(Unchecked)) {
      fprintf(stderr, "aborting due to unchecked expected value.\n");
      abort();
    }
  }
