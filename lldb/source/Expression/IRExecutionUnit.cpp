...
IRExecutionUnit::IRExecutionUnit(std::unique_ptr<llvm::LLVMContext> &context_up,
    ...
    : IRMemoryMap(target_sp), m_context_up(std::move(context_up)),
      ...
      m_symbol_resolver(std::move(symbol_resolver)), m_did_jit(false),
      m_will_interpret(false), m_function_load_addr(LLDB_INVALID_ADDRESS),
...
lldb::ExpressionResults IRExecutionUnit::Run(
  ...
  if (m_will_interpret)
    return RunInterpreted(args, exe_ctx, options, diagnostic_manager,
                          function_stack_bottom, function_stack_top);

  return RunUsingThreadPlan(args, exe_ctx, options, diagnostic_manager,
                            expression_sp, function_stack_bottom,
                            function_stack_top);
}

lldb::ExpressionResults IRExecutionUnit::RunInterpreted(
    llvm::ArrayRef<lldb::addr_t> args, ExecutionContext &exe_ctx,
    const EvaluateExpressionOptions &options,
  ...
  function_stack_bottom = m_interpreter_stack_bottom;
  function_stack_top = m_interpreter_stack_top;
...
lldb::ExpressionResults IRExecutionUnit::RunUsingThreadPlan(
    llvm::ArrayRef<lldb::addr_t> args, ExecutionContext &exe_ctx,
    const EvaluateExpressionOptions &options,
    DiagnosticManager &diagnostic_manager,
    lldb::UserExpressionSP &expression_sp, lldb::addr_t &function_stack_bottom,
